...
public class UIManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // 初始化生命图标（隐藏）
        if (healthIconsParent != null)
        {
            healthIconsParent.gameObject.SetActive(false);
        }
    }
    ...
    public void UpdateHealthIcons(int currentHealth)
    {
        if (healthIconsParent == null)
        {
            Debug.LogWarning("Health Icons Parent not assigned in UIManager!");
            return;
        }
        ...
        healthIconsParent.gameObject.SetActive(true);
        ...
        {
            Debug.LogWarning("Health Icon Prefab not assigned in UIManager!");
            return;
        }

        for (int i = 0; i < currentHealth; i++)
        {
            GameObject newIcon = Instantiate(healthIconPrefab, healthIconsParent);
            healthIcons.Add(newIcon);
        }
    }

    /// <summary>
    /// 设置 Level 2 的生命值（5条命）并显示图标
    /// </summary>
    public void SetLevel2Health()
    {
        UpdateHealthIcons(5);
    }
